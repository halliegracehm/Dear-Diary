...
export default function App() {
  ...
  async function handleAuth() {
    ...
    try {
      if(authMode==="register"){
        ...
        if(result?.length>0){const u=result[0];sessionStorage.setItem("myinnerminduser",JSON.stringify(u));setCurrentUser(u);setScreen("app");}
  ...
  function handleLogout(){sessionStorage.removeItem("myinnerminduser");setCurrentUser(null);setScreen("auth");setAuthForm({username:"",email:"",password:""});}

  async function upgradePlan(plan){
    if(!currentUser)return;
    await sbUpdate("users",`email=eq.${encodeURIComponent(currentUser.email)}`,{plan});
    const updated={...currentUser,plan};
    sessionStorage.setItem("myinnerminduser",JSON.stringify(updated));setCurrentUser(updated);
  }

  if(screen==="splash")return <Splash/>;
  if(screen==="auth")return <AuthScreen mode={authMode} form={authForm} error={authError} loading={authLoading} onChange={f=>setAuthForm(p=>({...p,...f}))} onSubmit={handleAuth} onToggle={()=>{setAuthMode(m=>m==="login"?"register":"login");setAuthError("");}} />;
...
function JournalApp({user,onLogout,onUpgradePlan}){
  ...
  async function loadPrompt(){
    ...
    try{const p=await getDailyPrompt();setDailyPrompt(p);sessionStorage.setItem("myinnermindPrompt_"+getTodayStr(),p);}
  ...
  async function saveEntry(){
    ...
    const entry={id:form.id||`${user.email}_${Date.now()}`,user_email:user.email,date:form.date,title:form.title,content:form.content,mood:form.mood,tags:form.tags,shared:form.shared,author_name:user.username,updated_at:Date.now()};
    await sbUpsert("entries",entry);await loadEntries();setView("list");
  }
  ...
  async function fetchInsight(entry){
    if(!isPro)return;setInsight("");setInsightLoading(true);
    try{setInsight(await getAIInsight(entry));}catch{setInsight("Couldn't load insight right now.");}finally{setInsightLoading(false);}
  }

  function startNew(prefill=""){setForm({id:null,date:getTodayStr(),title:"",content:prefill,mood:null,tags:[],shared:false});setEditMode(false);setView("write");}
  function startEdit(entry){setForm({id:entry.id,date:entry.date,title:entry.title,content:entry.content,mood:entry.mood,tags:entry.tags||[],shared:entry.shared||false});setEditMode(true);setView("write");}

  const filtered=entries.filter(e=>{
    const q=searchQuery.toLowerCase();
    return(!q||e.title?.toLowerCase().includes(q)||e.content?.toLowerCase().includes(q))&&(!filterTag||e.tags?.includes(filterTag));
  });
      ...
      <header style={S.header}>
        <div style={S.headerInner}>
          <button onClick={()=>{setTab("journal");setView("list");}} style={S.logoBtn}>
            <span style={{fontSize:22}}>🌿</span><span style={S.logoText}>My Inner Mind</span>
          </button>
          <nav style={{display:"flex",gap:4}}>
            {[["journal","📖 Journal"],["community","🌿 Community"],["pricing","💎 Plans"]].map(([t,label])=>(
              <button key={t} onClick={()=>{setTab(t);setView("list");}} style={{...S.navBtn,...(tab===t?S.navBtnActive:{})}}>{label}</button>
            ))}
          </nav>
      ...
      <main style={S.main}>
        {tab==="journal"&&view==="list"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              ...
              {isPro&&<button onClick={()=>exportToPDF(entries,user.username)} style={S.ghostBtn}>📄 Export PDF</button>}
            ...
            {loading?(<div style={{textAlign:"center",padding:"70px 0",color:"#b08060",fontStyle:"italic"}}>Loading your journal...</div>)
            :filtered.length===0?(<div style={{textAlign:"center",padding:"70px 0"}}><div style={{fontSize:52,marginBottom:14}}>📖</div><p style={{color:"#b08060",fontFamily:"'Lora',serif",fontStyle:"italic",fontSize:16}}>No entries yet — start writing!</p><button onClick={()=>startNew()} style={{...S.newBtn,marginTop:18}}>Write your first entry</button></div>)
            :(<div style={S.grid}>{filtered.map(entry=><EntryCard key={entry.id} entry={entry} onClick={()=>{setSelectedEntry(entry);setInsight("");setView("read");}}/>)}</div>)}
          </div>
        )}
        {tab==="journal"&&view==="write"&&(
          <div style={S.writeCard}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:4}}>
              {TAG_OPTIONS.map(t=><Chip key={t} active={form.tags.includes(t)} activeColor={TAG_COLORS[t]} onClick={()=>setForm(f=>({...f,tags:f.tags.includes(t)?f.tags.filter(x=>x!==t):[...f.tags,t]}))}>{t}</Chip>)}
            </div>
            <textarea style={S.textarea} rows={13} placeholder="What's on your mind today? Write freely..." value={form.content} onChange={e=>setForm(f=>({...f,content:e.target.value}))}/>
            {isPro&&(
        ...
        {tab==="journal"&&view==="read"&&selectedEntry&&(
          <div style={S.readCard}>
            ...
            <h1 style={{fontFamily:"'Lora',serif",fontSize:30,color:"#5a2e0e",marginBottom:14,fontWeight:600,lineHeight:1.2}}>{selectedEntry.title||"Untitled"}</h1>
            ...
            <p style={{fontFamily:"'Lora',serif",fontSize:17,lineHeight:1.9,color:"#6a4020",fontStyle:"italic",whiteSpace:"pre-wrap",marginBottom:32}}>{selectedEntry.content}</p>
            {isPro&&(
        ...
        {tab==="pricing"&&<PricingTab currentPlan={user.plan} onUpgrade={onUpgradePlan}/>}
      </main>
    </div>
  );
}
...
function CommunityTab({currentUser,isPro,onUpgrade}){
  ...
  async function loadPosts(){setLoading(true);try{const data=await sbGet("entries","shared=eq.true&order=updated_at.desc");setPosts(Array.isArray(data)?data:[]);}catch{setPosts([]);}finally{setLoading(false);}}
  if(!isPro)return(
    <div style={{textAlign:"center",padding:"80px 24px"}}>
      <div style={{fontSize:54,marginBottom:16}}>🌿</div>
      <h2 style={{fontFamily:"'Lora',serif",fontSize:26,color:"#5a2e0e",marginBottom:12}}>Community Feed</h2>
      <p style={{color:"#b08060",fontSize:16,maxWidth:400,margin:"0 auto 28px",lineHeight:1.7,fontStyle:"italic",fontFamily:"'Lora',serif"}}>Connect with others through shared reflections. Upgrade to Plus or Group to join.</p>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><h2 style={{fontFamily:"'Lora',serif",fontSize:26,color:"#5a2e0e"}}>🌿 Community Feed</h2><p style={{color:"#b08060",fontSize:13,marginTop:5}}>Entries shared by My Inner Mind members</p></div>
        <button onClick={loadPosts} style={S.ghostBtn}>↻ Refresh</button>
      </div>
      {loading?<div style={{textAlign:"center",padding:"60px 0",color:"#b08060",fontStyle:"italic"}}>Loading...</div>
      :posts.length===0?<div style={{textAlign:"center",padding:"60px 0"}}><div style={{fontSize:48,marginBottom:12}}>🌱</div><p style={{color:"#b08060",fontFamily:"'Lora',serif",fontStyle:"italic"}}>No shared entries yet. Be the first!</p></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:20,marginBottom:40}}>
        {Object.entries(PLANS).map(([key,plan])=>(
          <div key={key} style={{...S.pricingCard,...(currentPlan===key?{borderColor:plan.color,boxShadow:`0 4px 24px ${plan.color}33`}:{})}}>
            ...
            {currentPlan!==key&&<button onClick={()=>onUpgrade(key)} style={{...S.saveBtn,width:"100%",background:`linear-gradient(135deg,${plan.color}cc,${plan.color})`,boxShadow:`0 3px 14px ${plan.color}55`}}>{key==="free"?"Downgrade":"Upgrade →"}</button>}
          </div>
        ))}
      </div>
...
function EntryCard({entry,onClick,showAuthor}){
  ...
  return(
    <div style={{...S.card,cursor:onClick?"pointer":"default",transform:hovered?"translateY(-3px)":"",boxShadow:hovered?"0 10px 30px rgba(160,100,50,0.18)":"0 2px 12px rgba(160,100,50,0.10)"}} onClick={onClick} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
        ...
        <div style={{display:"flex",gap:5,alignItems:"center"}}>{entry.shared&&<span style={{fontSize:11,opacity:0.5}}>🌿</span>}{entry.mood&&<span style={{fontSize:18}}>{entry.mood.emoji}</span>}</div>
      </div>
